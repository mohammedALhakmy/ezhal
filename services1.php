<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
  <link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Noto+Kufi+Arabic:wght@100..900&display=swap" rel="stylesheet">
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <link rel="stylesheet" type="text/css" href="style.css"> 
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="https://code.jquery.com/jquery-3.6.4.min.js"></script>
    <script src="https://code.jquery.com/jquery-3.6.4.min.js"></script>
    <title>ezhal</title>
    
    <script src="main.js"></script>
        <!-- Icon Font Stylesheet -->
        <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.10.0/css/all.min.css" rel="stylesheet">
        <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.4.1/font/bootstrap-icons.css" rel="stylesheet">
    
        <!-- Libraries Stylesheet -->
        <link href="lib/animate/animate.min.css" rel="stylesheet">
        <link href="lib/owlcarousel/assets/owl.carousel.min.css" rel="stylesheet">
    
        <!-- Customized Bootstrap Stylesheet -->
        <link href="css/bootstrap.min.css" rel="stylesheet">
        <link href="css/style.css" rel="stylesheet">
</head>
<body>

    <body class="html">
    </head>

    <body class="html">
        <header class="header">
            <img src="img/loggoo.png" alt="شعار الموقع">
            <nav class="Navbar">
                <a href="homepage.php">الصفحة الرئيسية</a>
                <div class="has-submenu">
                    <a href="services1.php">الخدمات</a>
                    <div class="submenu" >
                        <div class="has2-submenu">
                            <a href="services.php">بريدإزهل</a>
                            <div class="submenu2" >
                                <a href="#" div class="sub2">  إرسال</a>
                                <a href="#" div class="sub2"> تتبع</a>
                            </div>
                        </div>
                        <a href="#" div class="sub"> الإنتقال بين المدن</a>
                        <a href="#" div class="sub">سائق خاص </a>
                        <a href="#" div class="sub">إيصال بنزين </a>
                        <a href="#" div class="sub">إيصال غاز </a>
                    </div>
                </div>
                <a href="team1.php">فريق العمل</a>
                <a href="aboutt1.php"> عن المنصة</a>
                <a href="contact1.php">  تواصل معنا</a>
                <a href="regist.php" style="color: #F57300; text-decoration: underline;">تسجيل</a>
                <a href="login.php" style="color: #F57300; text-decoration: underline;"> </a>
            </nav>
            <form onsubmit="event.preventDefault();" role="search">
                <label for="search"></label>
                <input id="search" type="search" placeholder="إبحث" autofocus required />
                <button type="submit">بحث</button>    
              </form>
         
                </div>
              </div>
            </header>

<div class="card1-group">
    <div class="card1">
        <div class="titlee"><p>سائق إزهل</p></div>
        <img src="img/card6.png">
        <div class="layer"></div>
        <div class="info"> 
            <p>نهدف إلى توفير تجربة آمنة وموثوقة لتوصيل الطلاب  حيث يمكن للأهل البحث وحجز سائق خاص لأطفالهم  بعناية وفقًا لمعايير الأمان والخدمة الرفيعة، مما يضمن رحلات آمنة</p>
            <button> للمزيد</button>
        </div>
    </div>

    <div class="card1">
        <div class="titlee"><p>توصيل غاز</p></div>
        <img src="img/card2.png">
       <div class="layer"></div>
       <div class="info">
        <p>نتيح لك طلب وتوصيل الغاز المنزلي مباشرةً إلى منزلك مما يجعل استخدام الغاز أمرًا أكثر يسرًا وفعالية</p>
        <button> للمزيد</button>
       </div>
    </div>

    <div class="card1">
        <div class="titlee"><p>إنتقال بين المدن</p></div>
        <img src="img/card3.png">
       <div class="layer"></div>
       <div class="info">
        <p>قدمنا لك رحلات مريحة وآمنة لتلبية احتياجات تنقلك بين مناطق المملكة بأمان وبسلاسة</p>
        <button> للمزيد</button>
       </div>
    </div>

    <div class="card1">
        <div class="titlee"><p>توصيل بنزين</p></div>
        <img src="img/card4.png">
       <div class="layer"></div>
       <div class="info">
        <p>نتيح لك طلب وتوصيل الوقود مباشرةً إلى موقعك موفرين لك الوقت والجهد</p>
        <button> للمزيد</button>
       </div>
    </div>

    <div class="card1">
        <div class="titlee"><p>بريد إزهل</p></div>
        <img src="img/card55.png">
       <div class="layer"></div>
       <div class="info">
        <p>يمكنك بسهولة تحديد المكان والوجهة عبر إزهل، وسنقوم بضمان وصول الطلب إلى وجهته بشكل سريع وآمن.</p>
        <button> للمزيد</button>
       </div>
    </div>
</div>
            <!-- Footer -->
            <footer id="footer" class="footer-1">
                <div class="main-footer widgets-dark typo-light">
                <div class="container">
                <div class="row">
                  
                <div class="col-xs-12 col-sm-6 col-md-3">
                <div class="widget subscribe no-box">
                  <h5 class="widget-title">
                    <img src="img/logoo2.png" alt="Company Logo" class="company-logo">
                </h5>
                </div>
                </div>
                <div class="col-xs-12 col-sm-6 col-md-3">
                <div class="widget no-box">
                <h5 class="widget-title">إنتقال سريع<span></span></h5>
                <ul class="thumbnail-widget">
                <li>
                <div class="thumb-content"><a href="#.">الرئيسية</a></div>	
                </li>
                <li>
                <div class="thumb-content"><a href="#.">خدماتنا</a></div>	
                </li>
                <li>
                <div class="thumb-content"><a href="#.">عن المنصة</a></div>	
                </li>
                <li>
                <div class="thumb-content"><a href="#.">تواصل معنا</a></div>	
                </li>
                <li>
              
                </ul>
                </div>
                </div>
                
                <div class="col-xs-12 col-sm-6 col-md-3">
                <div class="widget no-box">
                <h5 class="widget-title"> إنضم لفريق عمل إزهل<span></span></h5>
                <p>انضم إلينا وكن ركيزة أساسية في تطوير موقع إزهل</p>
                <a class="btn" href="process_subscription.php" target="_blank">إنضم الان</a>
                </div>
                </div>
                <div class="col-xs-12 col-sm-6 col-md-3">
                <div class="widget no-box">
                <h5 class="widget-title">تواصل معنا<span></span></h5>
                <p><a href="mailto:info@domain.com" title="glorythemes">ezhal2024@gmail.com</a></p>
                <ul class="social-footer2">
                <li class=""><a title="youtube" target="_blank" href="https://bit.ly/3m9avif"><img alt="youtube" width="30" height="30" src="data:image/png;base64,"></a></li>
                <li class=""><a href="https://www.facebook.com/" target="_blank" title="Facebook"><img alt="Facebook" width="30" height="30" src="data:image/png;base64,"></a></li>
                <li class=""><a href="https://twitter.com" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30" src="data:image/png;base64,"></a></li>
                <li class=""><a title="instagram" target="_blank" href="https://www.instagram.com/"><img alt="instagram" width="30" height="30" src="data:image/png;base64,"></a></li>
                </ul>
                </div>
                </div>
                
                </div>
                </div>
                </div>
                  
                <div class="footer-copyright">
                <div class="container">
                <div class="row">
                <div class="col-md-12 text-center">
                  <p>جميع الحقوق محفوظة لفريق عمل إزهل @2024ٍٍ</p>
                </div>
                </div>
                </div>
                </div>
                </footer>
     
                  <!-- Footer End -->
    <!-- Template Javascript -->
    <script src="js/main.js"></script>
</body>
</html>